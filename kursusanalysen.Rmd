...
```{r sex-dist-2, echo  = FALSE, warning=FALSE,message=FALSE}
tilmeldinger %>% 
  distinct(booking_id, .keep_all = TRUE) %>% 
  select(event_id, tilm_type, booking_id, køn) %>% 
  left_join(meta_data %>% distinct(id, .keep_all= TRUE), by = c("event_id" = "id")) %>% 
...
```{r lille-udvalg, echo = FALSE, message=FALSE}
...
## Antal tilmeldinger pr. kursus pr år

For folk der har svært ved at læse tabeller:

0 betyder ikke nødvendigvis at kurset har været en fiasko. Det kan også bare, helt enkelt, betyde
at kurset første gang blev udbudt i 2023. Og derfor var der 0 deltagere på det endnu ikke-eksisterende kursus i
2022.
...
meta_data %>% 
  ...
  right_join(tilm_pr_event, by = c("id" = "event_id")) %>% 
  select(id, title, start, registrants, waitlist, seats) |> 
  group_by(title, år = year(start))  |> 
  summarise(registrants = sum(registrants, na.rm = TRUE)) |> 
  pivot_wider(names_from = år, values_from = registrants, values_fill = 0) |> 
  arrange(desc(`2025`)) |> 
  gt()
```
...
meta_data %>% 
  ...
  filter(!str_detect(title, "værksted")) %>%
  select(title, start, seats, seats_taken, category_name) %>% 
  filter(!is.na(seats_taken)) %>% 
  mutate(år = year(start),
        periode = as.yearmon(start)) %>% 
  group_by(category_name, år) %>% 
  summarise(grad_fyldt = sum(seats_taken)/sum(seats)) %>% 
  mutate(grad_fyldt = percent(grad_fyldt, 
                              accuracy = 1, 
                              decimal.mark = ",")) %>% 
  pivot_wider(names_from = år, values_from = grad_fyldt, values_fill = "0%") |> 
  gt() %>% 
  tab_header(title = "Andel af pladser udbudt, fyldt") %>% 
  tab_footnote("MIDLERTIDIG FODNOTE: En række digital humaniora aktiviteter, der har været annonceret, men hvor tilmelding er foregået på anden vis, har ikke registrerede tilmeldinger. Tilsvarende er 'Åbent værksted' fjernet fra oversigten, da der her heller ikke er tilmelding.") 
```
...
tilmeldinger %>% 
  ...
  filter(year(registered_date) == 2025) %>% 
...
## Sammenligning mellem kursuskalendere

Belægningsgrader og sådan. Der er vi jo nødt til at have en ide om hvordan det ser ud for andre kursuskalendere.

Vi trækker ret sammenfattende data for fem kursuskalendere, for perioden 2022 til dato.
Kurser med 
Hvor mange kurser er det vi sammenligner imellem? Bemærk at ting i kalenderen
hvor der ikke er angivet et antal pladser man kan tilmelde sig, er pillet fra.
det fjerner eksempelvis alle åbne værksteder.

```{r}
sammenligning <- read_csv2("data/summeret_alle_kursuskalendere.csv")
# a data frame with all the annotation info
annotation <- data.frame(
   x = c(2024,2024.5),
   y = c(95,50),
   label = c("Datalab har mange kurser. Andre har flere", "Husk at 2025\n ikke er overstået endnu")
)
sammenligning |> 
  select(navn, year, antal) |> 
  ggplot(aes(x=  year, y = antal, color = navn)) +
  geom_line() +
  geom_text(data = annotation, aes(x,y,label=label),
            color = "darkred")+
  annotate("rect", xmin=c(2023.9), xmax=c(2024.1), ymin=c(70) , ymax=c(95), alpha=0.1, fill="blue")
```

Hvordan med belægningsgraden? Klarer vi os virkelig så ringe?
```{r}
annotation <- data.frame(
   x = c(2024.1,2024.5),
   y = c(.75,.50),
   label = c("Kalendrene med færrest kurser\n har højest belægningsgrad", "Men overordnet ligger vi vist OK")
)
sammenligning |> 
  select(navn, year, seats, seats_taken) |> 
  mutate(belægning = seats_taken/seats) |> 
  ggplot(aes(x=year, y = belægning, color = navn)) +
  geom_line() +
   geom_text(data = annotation, aes(x,y,label=label),
            color = "darkred")+
  annotate("rect", xmin=c(2024.9), xmax=c(2025.1), ymin=c(.75) , ymax=c(.77), alpha=0.1, fill="blue") +
  annotate("segment", x = 2025, xend = 2025, y = .46, yend = .65, colour = "pink", size=1, alpha=1, arrow=arrow(ends = "both"))
  ggtitle("belægningsgrad sammenlignet over tid mellem 5 kalendere") 
```
...
```{r}
sammenligning |> 
  select(navn, year, seats_taken) |> 
  ggplot(aes(x= year, y = seats_taken, color = navn)) +
  geom_line() +
  ggtitle("hvor mange tilmedlinger er der - udtryk for efterspørgslen")
```
...
```{r}
sammenligning |> 
  select(navn, year, seats_taken, antal) |> 
  mutate(snit = seats_taken/antal) |> 
  ggplot(aes(x=year, y = snit, color = navn)) +
  geom_line()
```

Jamen udbyder vi så ikke bare for mange pladser?

Måske, men det er der vist andre der også gør. 

```{r}
sammenligning |> 
  select(navn, year, seats) |> 
  ggplot(aes(x=year, y = seats, color = navn)) +
  geom_line()
```